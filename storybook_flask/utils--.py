...
def generate_storybook_pdf_util(data_file_path, uploaded_image_dir, output_pdf_path, 
    ...
    Args:
        ...
        layout_style (str): 布局样式，'image_top_text_bottom' 或 'image_left_text_right'。
    Returns:
    ...
    word_style = ParagraphStyle(
        ...
        fontSize=48,
        alignment=TA_CENTER, # 默认居中，但布局为左右时会覆盖
        spaceAfter=40,
    ...
    for index, row in df.iterrows():
        ...
        if layout_style == 'image_top_text_bottom':
            ...
            # 修正：wrapOn 的宽度应该与 drawOn 的宽度相同，并且是居中对齐的有效区域
            w_word, h_word = word_paragraph.wrapOn(c, width * 0.9, height) # 给一个稍窄的宽度，避免贴边
            ...
            # 调整文字样式为左对齐（针对左侧图片，右侧文字的布局）
            # 注意：这里我们创建一个新的样式实例，而不是直接修改父样式，以避免影响其他布局
            ...
            # 计算图片和文字的起始 X 坐标
            img_x_start = padding
            text_x_start = width / 2 + middle_gap / 2 # 右侧文字的起始 X 坐标

            # 居中垂直对齐图片和文字
            available_height = height - 2 * padding # 上下留白
            ...
            if img_path:
                try:
                    ...
                    img_y = (height - display_height) / 2 # 垂直居中
                    # 修正：图片在左侧区域内水平居中
            ...
            # 文字部分
            # 计算文字的总高度，包括单词、句子和它们之间的间距
            # 使用 wrapOn 计算在 text_section_width 宽度下，文本的实际高度
            word_paragraph = Paragraph(word, word_style_right_aligned)
            w_word, h_word = word_paragraph.wrapOn(c, text_section_width, available_height) 
            
            sentence_paragraph = Paragraph(sentence, sentence_style_right_aligned)
            w_sentence, h_sentence = sentence_paragraph.wrapOn(c, text_section_width, available_height)
            
            # 单词和句子之间的固定间距
            text_spacing = 20 # 调整这个值来控制单词和小句之间的距离
            total_text_height = h_word + text_spacing + h_sentence

            # 确保文字内容不会超出可用的垂直高度
            if total_text_height > available_height:
                print(f"警告: 文本内容在 '{layout_style}' 布局下超出可用高度。可能出现截断或重叠。")
                # 可以选择缩小字体大小或调整行间距，这里简单提示
                # 实际应用中可能需要更复杂的自适应逻辑
                pass

            # 计算文字块的起始 Y 坐标，使其在右侧区域内垂直居中
            text_y_start = (height - total_text_height) / 2
            
            # 修正：调整绘制顺序和Y坐标，确保单词在上面，句子在下面，且彼此不重叠
            # 单词的Y坐标
            word_y = text_y_start + h_sentence + text_spacing 
            word_paragraph.drawOn(c, text_x_start, word_y)
            
            # 小句的Y坐标
            sentence_y = text_y_start
            sentence_paragraph.drawOn(c, text_x_start, sentence_y)
